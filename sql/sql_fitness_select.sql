...
SELECT ZAN1._Fld898RRef AS ID, STAT._EnumOrder AS statuszan, ZAN1._Fld907 AS statprib, ZAN2._Fld850 AS starttime, ZAN2._Fld851 AS finishtime
...
FROM _InfoRg6849 AS CLRLX
    ...
    /* _Reference56 Справочник.ВидыКарт (выбираем тип карты клиента)*/
    LEFT JOIN _Reference56 AS TYPCARDX ON TYPCARDX._IDRRef = CARDSX._Fld2878RRef
    ...
    LEFT JOIN _Reference93 AS SUB ON SUB._Marked = 0x0 AND SUB._IDRRef = CARDS._Fld2879_RRRef
    LEFT JOIN T1 AS BINFILE ON BINFILE.ID = SUB._Fld3026_RRRef
    LEFT JOIN T2 ON T2.ID = CLRLX._Fld6851RRef
    LEFT JOIN T3 ON T3.ID = CLRLX._Fld6851RRef
WHERE CARDS._Fld2882 = @WILDCARD AND CARDS._Fld2878RRef = @IDSOP AND (CARDSX._Fld2878RRef IS NULL OR CARDSX._Fld2878RRef <> @IDSOP) AND CARDS._Marked = 0x0 AND CARDS._Fld2888 = 0x0 AND (DATEADD(yy, -2000, DOC1._Fld7489) > DATEADD(mm, -12, getdate()) OR TYPCARDX._Description LIKE '%аренд%')
