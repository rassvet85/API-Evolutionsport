	/*MSSQL 2019 - синхронизация всех сотрудников из 1С ЗУП/
...
WITH T1 AS 
(
	SELECT CONVERT(VARCHAR(32), s._Fld6975RRef, 2) AS ID, m._Description AS NAME, s._Code AS TABELNUMBER, org._Description AS ORGANIZATION, 
	CONCAT(org._Description, '|', REPLACE(REPLACE(REPLACE(REPLACE(e._Description, '( не СЭЗ)',''), '(не СЭЗ)',''), 'Звездный"-','Звездный"'), ' не СЭЗ', '')) AS DEPARTMENT, 
	e3._Description AS DOLGNOST, d._Fld22646 AS STAVKA, e5._EnumOrder AS STAVKANUM, ISNULL(LEN(f._Fld26060), 0) AS VERSIONFOTO, 
	...
	/* _Referencehttps://github.com/rassvet85/laravel_API_evolution126 Справочник.Организации (находим организацию в которой работает работник ИП Плошенко либо наш работник)*/
	...
	SELECT ID, NAME, TABELNUMBER, ORGANIZATION, DEPARTMENT, DOLGNOST, VERSIONFOTO, TIMES
	FROM T1 as M1
